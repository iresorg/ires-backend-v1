...
export class UserRepository implements IUserRepository {
    ...
    async update(id: string, body: Partial<IUser>): Promise<IUser> {
        ...
        if (savedUser.affected === 0) throw new UserNotFoundError(`User with id ${id} not found.`);

        const updatedUser = await this.userRepo.findOne({ where: { id } });
        if (!updatedUser) throw new UserNotFoundError(`User with id ${id} not found after update.`);
        return this.mapToIUser(updatedUser);
    }
   
    async delete(id: string): Promise<boolean> {
        const result = await this.userRepo.delete(id);
        if (result.affected === 0) throw new UserNotFoundError(`User with id ${id} not found.`);
        return true;
    }
